...
} from '../../../../../pages/Wallets/components/WalletConnect/WalletConnectModal/types';
...
const unassignLegacyClientEvents = (client: LegacyWalletConnect | null) => {
  ...
  client.off(WalletConnectEvent.SessionRequest);
  client.off(WalletConnectEvent.CallRequest);
};
...
export const LegacyWalletConnectContext = createContext<{
  ...
  legacyProposal?: SessionRequestParam | null;
...
const LegacyWalletConnectContextProvider = (props: PropsWithChildren) => {
  ...
  const [legacyProposalQueue, setLegacyProposalQueue] = useState<SessionRequestParam[]>([]); // Proposals should be limited to one, but keep it like this for now
  ...
  useEffect(() => {
    ...
    const onSessionRequest = (error: any, legacyProposal: WalletConnectMessage<SessionRequestParam>) => {
      if (error) {
    ...
    legacySignClient.on(WalletConnectEvent.SessionRequest, onSessionRequest);
    legacySignClient.on(WalletConnectEvent.CallRequest, onCallRequest);
