...
export const KrayonWithAuth0SDKProvider = (props: KrayonAuth0SDKProviderProps): JSX.Element | null => {
  const {
    children,
  ...
  const [sdkStatus, setSdkStatus] = useState<SDKReadyStatus>(krayonSdkInstance.status);
  ...
  useEffect(() => {
    ...
    Promise.all([getAccessTokenSilently(), getIdTokenClaims()]).then(async ([token, idTokenClaims]) => {
      if (!idTokenClaims) {
      ...
      krayonSdkInstance.start({ token, idTokenClaims, authProvider: 'auth0' });
    });
    ...
    <KrayonSdkClientContext.Provider value={krayonSdkInstance}>
      ...
      {!isAuth0Loading && (
        ...
        */}
          ...
          {!isSdkReady && (sdkNotReadyComponent ? sdkNotReadyComponent : children)}
        </>
      )}
